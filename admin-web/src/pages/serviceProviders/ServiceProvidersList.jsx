import React from 'react';

export default function ServiceProvidersList() {
  return (
    <div>
      <h1 style={{ color: '#38bdf8' }}>Service Providers Directory</h1>
      <p style={{ color: '#94a3b8' }}>Monitor independent contractors, logistics companies, and maintenance service listings.</p>
      <div style={{ padding: '20px', backgroundColor: '#1e293b', borderRadius: '8px', border: '1px solid #334155', color: '#cbd5e1' }}>
        No service providers registered yet.
      </div>
    </div>
  );
}
