...
export default function VendorSidebar({ activePage, setActivePage, isMobile, mobileMenuOpen, setMobileMenuOpen, onLogout }) {
  ...
  const filteredNav = NAV.map(item => {
    ...
    if (item.key === 'inventory') {
      if (vendorType === 'raw-material') return { ...item, label: "Inventory", icon: Package, exclusive: true };
      if (vendorType === 'manpower') return { ...item, label: "Staff Roster", icon: Users };
      if (vendorType === 'service') return { ...item, label: "Service Catalog", icon: Wrench };
      if (vendorType === 'marketing') return { ...item, label: "Campaigns", icon: Megaphone };
    }
    if (item.key === 'deliveries') {
      if (vendorType === 'manpower') return { ...item, label: "Deployments", icon: Users };
      if (vendorType === 'service') return { ...item, label: "Dispatches", icon: Wrench };
      if (vendorType === 'marketing') return { ...item, label: "Deliverables", icon: Megaphone };
    }
    return item;
  });
    ...
    <View style={styles.sidebarInner}>
      <View style={styles.topSection}>
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatarBox}>
            <Text style={styles.avatarInitials}>
              {(user?.registration?.bizName || user?.businessName || user?.bizName || user?.name || 'VP').substring(0, 2).toUpperCase()}
            </Text>
          </View>
          <View style={styles.profileInfo}>
            <Text style={styles.profileName} numberOfLines={1}>
              {user?.registration?.bizName || user?.businessName || user?.bizName || user?.name || 'Business Partner'}
            </Text>
        ...
        <ScrollView style={styles.scrollArea} contentContainerStyle={{ paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
          {filteredNav.map((item) => <NavBtn key={item.key} item={item} />)}
        </ScrollView>
      </View>
