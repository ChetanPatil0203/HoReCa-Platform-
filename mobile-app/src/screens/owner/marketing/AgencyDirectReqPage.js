...
export default function AgencyDirectReqPage({ agency, onBack, onHome, visible = true }) {
  ...
  const handleSelectMarketingType = (type) => {
    ...
    setServiceNeeded(''); // Reset service needed when marketing type changes
    if (errors.marketingType) setErrors(prev => ({ ...prev, marketingType: null }));
    if (errors.serviceNeeded) setErrors(prev => ({ ...prev, serviceNeeded: null }));
  ...
  const validateForm = () => {
    ...
    if (!serviceNeeded) errs.serviceNeeded = 'Service needed is required';
  ...
  const handleConfirmSubmit = async () => {
    try {
      ...
      await createRequirementApi({
        ownerId,
        supplierId: agency?.id,
        type: 'marketing',
        requestType: 'direct',
        title: requirementTitle.trim(),
        description: description.trim(),
        budget: `₹${minBFormatted} - ₹${maxBFormatted}`,
        location: targetAudience.trim() || agency?.location || 'Not Specified',
        extraData: {
          marketingType,
          serviceNeeded,
      ...
      setToastMessage('Direct request sent successfully.');
  ...
  return (
    ...
    >
      <View style={styles.backdrop}>
        <View style={[
          ...
          <View style={styles.popupHeader}>
            <View style={{ flex: 1 }}>
              <Text style={styles.headerTitle}>Send Direct Request</Text>
              <Text style={styles.headerSubtitle} numberOfLines={1}>
                Share your requirement with the selected marketing agency.
              </Text>
            </View>
          ...
          >
            ...
            <View style={styles.fieldGroup}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Text style={serviceNeeded ? styles.inputText : styles.placeholderText}>
                  {serviceNeeded || 'Select service'}
                </Text>
                <ChevronDown size={18} color={TEXT_SECONDARY} />
              </TouchableOpacity>
              {errors.serviceNeeded && <Text style={styles.errorInlineText}>{errors.serviceNeeded}</Text>}
            </View>
            ...
            <View style={styles.fieldGroup}>
              ...
              <TextInput
                ...
                placeholder="Briefly explain your objective and expected output."
                placeholderTextColor={TEXT_SECONDARY}
                multiline
                numberOfLines={4}
                maxLength={400}
            ...
            {/* 6. DURATION & 7. PREFERRED START DATE (PAIRED ROW) */}
            <View style={styles.fieldGroup}>
              <View style={[styles.pairedRow, isSmallScreen && styles.pairedRowStacked]}>
            ...
            {/* 9. REFERENCE ATTACHMENT */}
            <View style={styles.fieldGroup}>
              <Text style={styles.fieldLabel}>Reference Attachment</Text>
              <Text style={styles.helperText}>Optional — upload a sample image, PDF or campaign brief.</Text>
              ...
              {attachedFile ? (
                ...
                <TouchableOpacity style={styles.uploadArea} onPress={handlePickFile} activeOpacity={0.7}>
                  ...
                  <Text style={styles.uploadAreaSub}>Sample brief, PDF or image</Text>
                </TouchableOpacity>
              )}
            </View>
      ...
      {/* 2. Service Needed Dropdown Modal */}
      <Modal visible={servicePickerVisible} transparent animationType="fade" onRequestClose={() => setServicePickerVisible(false)}>
        <Pressable style={styles.pickerOverlay} onPress={() => setServicePickerVisible(false)}>
          <View style={styles.pickerMenu}>
            <View style={styles.pickerHeader}>
              <Text style={styles.pickerHeaderTitle}>Select Service ({marketingType})</Text>
            ...
            <ScrollView style={{ maxHeight: 320 }}>
              {currentServiceOptions.map((srv) => {
                const isSelected = serviceNeeded === srv;
                return (
                  <TouchableOpacity
                    key={srv}
                    style={[styles.pickerItem, isSelected && styles.pickerItemActive]}
                    onPress={() => {
                      setServiceNeeded(srv);
                      if (errors.serviceNeeded) setErrors(prev => ({ ...prev, serviceNeeded: null }));
      ...
      <Modal visible={datePickerVisible} transparent animationType="fade" onRequestClose={() => setDatePickerVisible(false)}>
        <Pressable style={styles.pickerOverlay} onPress={() => setDatePickerVisible(false)}>
          <View style={styles.pickerMenu}>
            <View style={styles.pickerHeader}>
              <Text style={styles.pickerHeaderTitle}>Select Preferred Start Date</Text>
      ...
      <Modal visible={showConfirmModal} transparent animationType="fade" onRequestClose={() => setShowConfirmModal(false)}>
        <View style={styles.confirmBackdrop}>
          <View style={styles.confirmCard}>
            <Text style={styles.confirmTitle}>Send this direct request?</Text>
            <Text style={styles.confirmSub}>Review your direct request details before sending.</Text>
            ...
            <View style={styles.confirmSummaryBox}>
              ...
              <View style={styles.confirmRow}>
                ...
                <Text style={styles.confirmVal}>{serviceNeeded}</Text>
              </View>
              <View style={styles.confirmRow}>
                <Text style={styles.confirmLabel}>Agency Name</Text>
                <Text style={styles.confirmVal}>{agency?.name || 'Selected Marketing Agency'}</Text>
              </View>
              <View style={styles.confirmRow}>
            ...
            <View style={styles.confirmActionsRow}>
              ...
              >
                {isSubmitting ? (
                  ...
                  <Text style={styles.confirmSendText}>Confirm & Send</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
