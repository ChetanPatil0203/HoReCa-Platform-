...
export default function CampaignRequestsPage({ onBack, onViewResponses, onTrack }) {
  ...
  const loadRequests = async () => {
    try {
      ...
      if (res.success) {
        // Filter only marketing requirements
        const filtered = (res.data || [])
          ...
          .map(r => ({
            ...
            _rawId: r.id,
            title: r.title,
            type: r.requestType === 'direct' ? 'Direct' : 'Broadcast',
            responses: 0,
            budget: r.budget || '—',
            date: new Date(r.createdAt).toLocaleDateString('en-IN'),
            status: r.status === 'pending' ? 'Responses' : r.status.charAt(0).toUpperCase() + r.status.slice(1)
          }));
        setRequests(filtered);
      }
    } catch (err) {
      console.error('Failed to load owner marketing requirements:', err);
    } finally {
      setLoading(false);
    }
  };
