...
const NAVY = '#081A3A';
const STORAGE_KEY = 'hrc_read_notifications_manpower';
...
export default function ManpowerNotificationsPage() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      if (dbRes?.success && Array.isArray(dbRes.data) && dbRes.data.length > 0) {
        ...
        // Fallback sync from Live Vendor Job Requirements & Orders
        const [reqsRes, ordersRes] = await Promise.all([
          supplierId ? fetchVendorRequirements(supplierId).catch(() => []) : Promise.resolve([]),
          supplierId ? fetchVendorOrders(supplierId).catch(() => []) : Promise.resolve([])
        ]);

        const reqList = reqsRes?.data || reqsRes || [];
        const ordersList = ordersRes?.data || ordersRes || [];
        const list = [];

        if (Array.isArray(reqList) && reqList.length > 0) {
          reqList.forEach((req, idx) => {
            const timeStr = req.createdAt ? new Date(req.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Recently';
            const shortId = (req.id || `REQ-${idx + 101}`).toString().toUpperCase();
            const role = req.title || req.role || 'Staff Member';
            const bizName = req.owner?.bizName || 'HoReCa Establishment';
            const count = req.extraData?.numberOfStaff || req.staffRequired || '1';

            list.push({
              id: `req-${req.id || idx}`,
              type: 'Requirements',
              title: `New Direct Request: ${role}`,
              message: `${bizName} sent a job requirement for ${count} ${role}(s). Budget: ${req.budget || 'Custom'}.`,
              createdAt: req.createdAt || new Date().toISOString(),
              time: timeStr,
              isRead: req.status === 'closed' || req.status === 'completed',
            });
          });
        }
        ...
        if (Array.isArray(ordersList)) {
          ordersList.forEach((ord, idx) => {
            const timeStr = ord.createdAt ? new Date(ord.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '09:00 AM';
            const shortId = (ord.id || '').toString().slice(-4).toUpperCase();
            
            list.push({
              id: `man-${ord.id || idx}`,
              type: 'Orders',
              title: `Staff Request Update: #${shortId}`,
              message: `Restaurant ${ord.owner?.bizName || 'Bistro Deluxe'} requested staff. Status: ${ord.status || 'Assigned'}.`,
  ...
  const handleNotificationPress = async (item) => {
    ...
    try {
      ...
      if (userId && !item.id.toString().startsWith('man-') && !item.id.toString().startsWith('req-')) {
